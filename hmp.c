...
{
#if 0
	fprintf((FILE*)p, "%x\t%x\t%llx\t%x\t%x\t%x\t%x\t%s\n", 
			node->key, node->size, node->type, node->used_size,
			node->ret, node->cr3, node->esp, node->obj_name);
#endif
}
#endif
...
void set_dump(void);
void hmp_objdump(Monitor *mon, const QDict *qdict)
{
	set_dump();
}
