                             ...
                             int error_code, target_ulong next_eip, int is_hw)
{
	...
	if(intno == 0x80) {
		...
		if(syscall_intr(get_kernel_esp() & 0xffffe000)) {
			inc_intr();
		}
	}
#endif
...
//jzeng
	...
	if(is_proc_intr_exist(esp) && is_all_retaddr_exist(esp)) {
		...
		if(pemu_exec_stats.PEMU_iret_target_pc < 0xc0000000) {
			del_proc_intr(esp);
			delete_all_retstacks(esp);
		} else {
			desr_proc_intr(esp);
		}
	} else {
		//printf("%x %x\n", is_proc_intr_exist(esp), is_all_retaddr_exist(esp));
		//assert(0);
	}
#endif

#ifdef TRACE_SYSCALL_OBJ
	//printf("esp:%x iret:%x iret_target:%x\n", get_kernel_esp() & 0xffffe000,
	//		g_pc, pemu_exec_stats.PEMU_iret_target_pc);
...
#ifdef PEMU_DEBUG
		struct CPUX86State* env=(struct CPUX86State*)(first_cpu->env_ptr);
		fprintf(stderr, "sysenter system call:%d eip=%x\n", env->regs[R_EAX], env->eip);
#endif
	}
...
#ifdef TRACE_SYSCALL_OBJ
	syscall_enter(get_kernel_esp() & 0xffffe000, 2);
#endif
//end
...
#ifdef TRACE_SYSCALL_OBJ
	//syscall_exit(get_kernel_esp() & 0xffffe000, 2);
#endif
//end
}
