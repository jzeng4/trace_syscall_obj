...
{
#ifdef MYCPUID
	if(env->regs[R_ECX] == 0x12345678) {
		if(env->regs[R_EDX] == 0) {
			uninit_syscall_trace();
			printf(stderr, "end tracing syscall\n");
		} else {
			printf(stderr, "start tracing syscall %d\n", env->regs[R_EDX] & 0x0000ffff);
			init_syscall_trace(env->regs[R_EDX] & 0x0000ffff, (env->regs[R_EDX] >> 16) & 0x0000ffff);
		}
		return;
	}
#endif
